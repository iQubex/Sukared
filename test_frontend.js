...
assert(changelog.includes('Versioned Luavex output signature'), 'versioned output signature is missing from the changelog');
...
for (const feature of ['Virtualization', 'VM Protection', 'String Protection', 'Constant Protection', 'Integrity Protection', 'Minify Output']) assert(ui.includes(feature), `missing protection feature: ${feature}`);
assert(!ui.includes("toggle('Luavex Signature'"), 'mandatory Luavex signature must not be exposed as a setting');
assert(!ui.includes('Protection Profile') && !ui.includes('profileGrid'), 'profile UI is still present');
assert(!ui.includes('Obfuscation Engine') && !ui.includes('customSelect(draft.engine'), 'engine selector is still present');
...
assert(!/\b(?:Legacy|Experimental Engine)\b/i.test(publicBundle), 'public bundle exposes retired engine terminology');
